$(function () {
    //左侧fix楼层拦，移动改变innerhtml
    var fixleft = $('.fix-floor');
    for(a in fixleft){
        fixleft.eq(a).attr('fix-data',a)
        fixleft.eq(a).click(function(){
            var k = $(this).attr('fix-data')
            var high = $('.floor').eq(k).offset().top;
            console.log(high)
            $('html,body').animate({ scrollTop: high }, 'slow');
        })
        fixleft.eq(a).hover(function(){
            var k = $(this).attr('fix-data')
            $(this).css('color','red')
            if(k === '0'){
                fixleft.eq(k).html('服装')
            }
            if (k === '1') {
                fixleft.eq(k).html('美妆')
            }
            if (k === '2') {
                fixleft.eq(k).html('手机')
            }
            if (k === '3') {
                fixleft.eq(k).html('电器')
            }
            if (k === '4') {
                fixleft.eq(k).html('数码')
            }
        },function(){
            var k = $(this).attr('fix-data')
            $(this).css('color', 'gray')
            if (k === '0') {
                fixleft.eq(k).html('1f')
            }
            if (k === '1') {
                fixleft.eq(k).html('2f')
            }
            if (k === '2') {
                fixleft.eq(k).html('3f')
            }
            if (k === '3') {
                fixleft.eq(k).html('4f')
            }
            if (k === '4') {
                fixleft.eq(k).html('5f')
            }
        })}
